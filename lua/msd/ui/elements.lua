function MSD.WorkSpacePanel(parent, title, wd, hd, a_ignore)
	...
	child.clsBut = MSD.IconButton(child, MSD.Icons48.cross, child:GetWide()-34, 10, 25, nil, MSD.Config.MainColor["r"], function() panel.Close() end)
...
function MSD.IconButton(parent, mat, x, y, s, color, color2, func)
	...
	if x == "static" then
		button.StaticScale = { w = s, fixed_h = s, minw = s, minh = s }
	end
	button:SetSize( s, s )
	button:SetText( "" )
...
function MSD.TextEntry(parent, x, y, w, h, text, label, value, func, auto_update, focuse_update, multi, num)
	...
	if focuse_update then
		Entry.OnFocusChanged = function( self, gained ) if !gained then func(self, self:GetValue()) end end
	end
...
function MSD.ColorSelectBut(parent, x, y, w, h, color, func)
	...
	if x == "static" then
		button.StaticScale = { w = w, fixed_h = h, minw = 10, minh = h }
	else
		button:SetSize( w, h)
	end
	button.alpha = 0
	button.Paint = function( self, w, h )
		...
		if (self.hover or self.hovered) and !self.disabled then
			self.alpha = Lerp(FrameTime() * 5, self.alpha, 1)
		else
			self.alpha = Lerp(FrameTime() * 5, self.alpha, 0)
		end
...
function MSD.ButtonIcon(parent, x, y, w, h, text, icon, func, func2, color)
	...
	if x == "static" then
		button.StaticScale = { w = w, fixed_h = h, minw = 50, minh = h }
	else
		button:SetSize( w, h)
	end
	button.alpha = 0
...
function MSD.BoolSlider(parent, x, y, w, h, text, var, func)
	...
	button.OnCursorEntered = function( self ) self.hover = true end
	button.OnCursorExited = function( self ) self.hover = false end
	button.DoClick = function(self) if self.disabled then return end self.var = !self.var func(self, self.var) end
	if !x or !y then
		parent:AddItem(button)
	end
	return button
...
function MSD.ComboBox(parent, x, y, w, h, label, val)
	...
	if x == "static" then
		ComboBox.StaticScale = { w = w, fixed_h = h, minw = 150, minh = h }
	else
