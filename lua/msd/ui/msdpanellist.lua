...
function PANEL:EnableVerticalScrollbar()
	...
	self.VBar.Paint = function( s, w, h ) draw.RoundedBox( 4, 3, 13, 8, h-24, sdw) end
	self.VBar.btnUp.Paint = function( s, w, h ) end
	self.VBar.btnDown.Paint = function( s, w, h ) end
	self.VBar.btnGrip.Paint = function( s, w, h ) draw.RoundedBox( 4, 5, 0, 4, h+22, sdw) end
...
function PANEL:ClearEX( ex_panel )
	...
	if ( !IsValid( ex_panel ) ) then return end
		...
		for k, panel in pairs( self.Items ) do
			...
			if ( panel:IsVisible() ) then
				...
				if panel.StaticScale then
					...
					if panel.StaticScale.minw > w  then w = panel.StaticScale.minw end
					if panel.StaticScale.minh > h  then h = panel.StaticScale.minh end
				...
				if ( x > self.Padding && ( x + w > (self:GetWide() - vbar) || OwnLine ) ) then
